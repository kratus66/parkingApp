...
export function GaugeMeter({ value, max, size = "lg", showLabel = true, label, sublabel }: GaugeMeterProps) {
  const percentage = Math.min((value / max) * 100, 100)

  const { color, status } = useMemo(() => {
    if (percentage <= 50) return { color: "var(--gauge-green)", status: "Fluido" }
    if (percentage <= 80) return { color: "var(--gauge-yellow)", status: "Medio" }
    return { color: "var(--gauge-red)", status: "Crítico" }
  }, [percentage])
